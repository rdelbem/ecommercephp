...
class User extends Model {
	...
	public static function getForgot( $email, $inadmin = true ) {
		...
		if ( count( $results ) === 0 ) {
			...
			if ( count( $results2 ) === 0 ) {
				...
				$code = openssl_encrypt( $dataRecovery['idrecovery'], 'AES-128-CBC', pack( 'a16', self::SECRET ), 0, pack( 'a16', self::SECRET_IV ) );

				$code = base64_encode( $code );

				if ( $inadmin === true ) {

					$link = "http://www.ecommercephp.com/admin/forgot/reset?code=$code";

				} else {

					$link = "http://www.ecommercephp.com/forgot/reset?code=$code";
	...
	public static function validForgotDecrypt( $code ) {
		...
		$idrecovery = openssl_decrypt( $code, 'AES-128-CBC', pack( 'a16', self::SECRET ), 0, pack( 'a16', self::SECRET_IV ) );
